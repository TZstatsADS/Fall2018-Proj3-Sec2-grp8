...
image_file_numbers <- split(extra_label$Image, extra_label$Label)
image_files0 <- gsub(".jpg","",image_file_numbers[["0"]])
image_files1 <- gsub(".jpg","",image_file_numbers[["1"]])
image_files2 <- gsub(".jpg","",image_file_numbers[["2"]])
```
...
Calculate the PSNR of genreated high resolution images from test images
```{r PSNR on test images}
source("../lib/getPSNR.R")
test_true_HR_dir <- paste(test_dir, "HR_true/", sep="")
test_perf <- getPSNR(test_true_HR_dir, test_HR_dir)
```
