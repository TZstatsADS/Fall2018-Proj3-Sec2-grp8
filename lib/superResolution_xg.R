...
applySuperResolution <- function(file_num, LR_dir, HR_dir, modelList, file_names){
  imgLR <- readImage(paste0(LR_dir,  "img", "_", sprintf("%04d", file_names[file_num]), ".jpg"))
  pathHR <- paste0(HR_dir,  "img", "_", sprintf("%04d", file_names[file_num]), ".jpg")
...
superResolution <- function(LR_dir, HR_dir, modelList, file_names){
  
  ### Construct high-resolution images from low-resolution images with trained predictor
  
  ### Input: a path for low-resolution images + a path for high-resolution images 
  ###        + a list for predictors
  ###        + a vector for filenames number in one category
  ...
  llply(as.list(1:n_files), applySuperResolution, LR_dir=LR_dir, 
        HR_dir=HR_dir, modelList=modelList,file_names=file_names, .parallel = T)
}